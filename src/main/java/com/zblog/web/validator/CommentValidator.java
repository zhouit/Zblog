...
public class CommentValidator{
  ...
  public static MapContainer validate(Comment comment){
    ...
    }else if(StringUtils.isBlank(comment.getEmail()) || !CommRegular.EMAIL.matches(comment.getEmail())){
      result.put("msg", "邮箱格式不合法");
    }else if(StringUtils.isBlank(comment.getUrl())){
      result.put("msg", "站点格式不合法 ");
    }else if(StringUtils.isBlank(comment.getCreator())){
      result.put("msg", "需填写用户名");
